...
export async function POST(req: NextRequest) {
  if (!process.env.ANTHROPIC_API_KEY) {
    return NextResponse.json(
      { error: "Letter assist is not configured yet." },
      { status: 503 }
    );
  }
  ...
  if (!barrier || typeof barrier !== "string" || barrier.length > 2000) {
    return NextResponse.json({ error: "Describe the barrier (up to 2000 characters)." }, { status: 400 });
  }
  ...
  return NextResponse.json({ draft: text && "text" in text ? text.text : "" });
}
